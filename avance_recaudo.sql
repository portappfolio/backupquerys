...
--select *
--from tbl_slv_fe_facturacion
--where cohorte_renovacion in ('2023-01-01','2023-02-01','2023-03-01','2023-04-01','2023-05-01','2023-06-01')

select distinct
    b.serial
    , coalesce(f.`_2023`,0) `2023`
    , coalesce(f.Pagos_2023,0) `$$ 2023`
    , case when b.valor_renovacion = coalesce(f.Pagos_2023,0) then 'Pago Completo' when b.valor_renovacion < coalesce(f.Pagos_2023,0) then 'Plan Superior' else 'Con Descuento' end clasificacion_pago
    , nc.hid id_nc
    , nc.motivo motivo_nc
...
where
--b.uso != 'Sin Uso'
--(b.cohorte_renovacion IN ('2023-03-01','2023-04-01') and coalesce(f.`_2023`,0) = 0)
--or (b.cohorte_renovacion = '2023-12-01' and b.tipo = 'PPTO 2022' and coalesce(f.`_2022`,0) = 0)
b.cohorte_renovacion in ('2023-03-01','2023-04-01')
